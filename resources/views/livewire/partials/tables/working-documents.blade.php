<div class="overflow-x-auto">
    <table class="min-w-full text-[13.5px]">
        ...
        <tbody>
            @forelse($saftData['working_documents'] as $i => $doc)
                <tr class="border-b border-[var(--color-border-warm)] dark:border-[#332f2a] hover:bg-blue-50/40 dark:hover:bg-blue-900/10 {{ $i % 2 === 1 ? 'bg-[var(--color-surface-2)] dark:bg-[#262320]' : '' }}">
                    <td class="px-4 py-3 font-semibold text-[var(--color-text-strong)] dark:text-[#f4f1ec]">{{ $doc['DocumentNumber'] }}</td>
                    <td class="px-4 py-3 text-[var(--color-text-muted)] dark:text-[#b3ada3]">{{ $doc['WorkType'] }}</td>
                    <td class="px-4 py-3">
                        @php
                            $wBadge = $doc['WorkStatus'] === 'N'
                        ...
                        @endphp
                        <span class="inline-flex items-center px-2 py-[3px] rounded-[7px] text-[11.5px] font-semibold border {{ $wBadge }}">
                            {{ $doc['WorkStatus'] === 'N' ? __('saft.status_normal') : ($doc['WorkStatus'] === 'A' ? __('saft.status_cancelled') : $doc['WorkStatus']) }}
                        </span>
                    </td>
                    <td class="px-4 py-3 font-mono text-[var(--color-text-muted)] dark:text-[#b3ada3] tabular-nums">{{ $doc['WorkDate'] }}</td>
                    <td class="px-4 py-3 text-[var(--color-text-muted)] dark:text-[#b3ada3]">{{ $doc['CustomerID'] }}</td>
                    <td class="px-4 py-3 text-right font-mono text-[var(--color-text-muted)] dark:text-[#b3ada3] tabular-nums">{{ number_format((float)$doc['NetTotal'], 2, ',', '.') }} &euro;</td>
                    <td class="px-4 py-3 text-right font-mono text-[var(--color-text-muted)] dark:text-[#b3ada3] tabular-nums">{{ number_format((float)$doc['TaxPayable'], 2, ',', '.') }} &euro;</td>
                    <td class="px-4 py-3 text-right font-mono font-semibold text-[var(--color-text-strong)] dark:text-[#f4f1ec] tabular-nums">{{ number_format((float)$doc['GrossTotal'], 2, ',', '.') }} &euro;</td>
                </tr>
            @empty
                <tr><td colspan="8" class="px-4 py-10 text-center text-[var(--color-text-dim)] dark:text-[#827c72]">{{ __('saft.empty_working_docs') }}</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
